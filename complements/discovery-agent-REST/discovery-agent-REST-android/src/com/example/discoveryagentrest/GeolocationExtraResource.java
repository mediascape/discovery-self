...
public class GeolocationExtraResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		String answer="";
		Log.d("MyApp", "GeolocationExtra");
		try {
			JSONObject mainObj = new JSONObject();
			JSONArray ja = new JSONArray();
			JSONObject jo = new JSONObject();
			if(ServiceBoot.gpsSensor){
				jo.put("latitude", ServiceBoot.latitude);
				ja.put(jo);
				JSONObject jo1 = new JSONObject();
				jo1.put("longitude", ServiceBoot.longitude);
				ja.put(jo1);
				mainObj.put("extra", ja);
			}else{
				mainObj.put("extra", "Error geting geolocation information.");
			}
			if(getReference().getRemainingPart().contains("callback")) answer="extraGeolocation("+mainObj.toString()+")";
