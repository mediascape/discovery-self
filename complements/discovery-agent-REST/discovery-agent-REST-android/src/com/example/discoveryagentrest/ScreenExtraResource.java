...
public class ScreenExtraResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		try {
			...
			if(ServiceBoot.screenWidth!=0 && ServiceBoot.screenHeight!=0){
				jo.put("width", ServiceBoot.getScreenWidth(ServiceBoot.context));
				ja.put(jo);
				JSONObject jo1 = new JSONObject();
				jo1.put("height",  ServiceBoot.getScreenHeight(ServiceBoot.context));
				ja.put(jo1);
				mainObj.put("extra", ja);
			}else{
				mainObj.put("extra", "Error geting screen information.");
			}

			if(getReference().getRemainingPart().contains("callback")) answer="extraScreen("+mainObj.toString()+")";
    ...
    public String toString(String value) {
		...
		try {
			...
			JSONObject jo = new JSONObject();
			if(ServiceBoot.screenWidth!=0 && ServiceBoot.screenHeight!=0){
				jo.put("width", ServiceBoot.screenWidth);
				ja.put(jo);
				JSONObject jo1 = new JSONObject();
				jo1.put("height", ServiceBoot.screenHeight);
				ja.put(jo1);
				mainObj.put("extra", ja);
			}else{
				mainObj.put("extra", "Error geting screen information.");
