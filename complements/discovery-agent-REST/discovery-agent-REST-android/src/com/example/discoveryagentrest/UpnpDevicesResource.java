...
public class UpnpDevicesResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		try {
			if(ServiceBoot.listAdapter.size()>0){
				...
				if(getReference().getRemainingPart().contains("callback")) answer="devicesUpnp("+mainObj.toString()+")";
				...
				if(getReference().getRemainingPart().contains("callback")) representation = new StringRepresentation("devicesUpnp({\"devices\":\"The number of device is 0.\"})", MediaType.APPLICATION_JSON);
