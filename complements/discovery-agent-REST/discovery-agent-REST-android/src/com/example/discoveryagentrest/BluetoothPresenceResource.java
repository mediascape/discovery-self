...
public class BluetoothPresenceResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		String answer="";
		Log.d("MyApp", "BluetoothPresence");
		try {
			if(BluetoothService.bluetoothSensor != null)
			{
				JSONObject mainObj = new JSONObject();
				if(BluetoothService.bluetoothSensor.isEnabled()){
					mainObj.put("presence", true);
				}else{
					mainObj.put("presence", false);
				}
				if(getReference().getRemainingPart().contains("callback")) answer="presenceBluetooth("+mainObj.toString()+")";
				else answer=mainObj.toString();
				Log.d("MyApp", "presence: "+answer);
				representation = new StringRepresentation(answer, MediaType.APPLICATION_JSON);
			}else{
				if(getReference().getRemainingPart().contains("callback")) representation = new StringRepresentation("presenceBluetooth({\"presence\":\"The number of device is 0.\"})", MediaType.APPLICATION_JSON);
				else representation = new StringRepresentation("{\"presence\":\"The number of device is 0.\"}", MediaType.APPLICATION_JSON);
			}
			/*JSONObject mainObj = new JSONObject();
			if(BluetoothService.bluetoothSensor != null){
				mainObj.put("presence", "true");
			...
			if(getReference().getRemainingPart().contains("callback")) answer="presence(The number of device is 0.)";
			else answer=mainObj.toString();*/
			representation = new StringRepresentation(answer, MediaType.APPLICATION_JSON);
