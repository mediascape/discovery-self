...
public class UpnpActionsResource extends ServerResource {
	/*
	 * toString
	 * 
	...
	public Representation doGet() {
		...
		String answer="";
		if(getReference().getRemainingPart().contains("service=")&&getReference().getRemainingPart().contains("device=")){
			Log.d("MyApp", "UPnPActions");
			try{
						...
						for(org.teleal.cling.model.meta.Service service : device.getServices()){
							...
							if(serviceType.equals(serviceName)){
								if(service.getActions().length>0){
									...
									//answer="actions("+mainObj.toString()+")";
									if(getReference().getRemainingPart().contains("callback")) answer = "actionsUpnp("+mainObj.toString()+")";
									else answer = mainObj.toString();
									Log.d("MyApp", "actions: "+answer);
									representation = new StringRepresentation(answer, MediaType.APPLICATION_JSON);
								}else {
									if(getReference().getRemainingPart().contains("callback")) representation = new StringRepresentation("actionsUpnp({\"service\":\"The service has no actions.\"})", MediaType.APPLICATION_JSON);
									else representation = new StringRepresentation("{\"service\":\"The service has no actions.\"}", MediaType.APPLICATION_JSON);
								}
							}
						}
					}else {
							if(getReference().getRemainingPart().contains("callback")) representation = new StringRepresentation("actionsUpnp({\"service\": \"Selected device is not in the list.\"})", MediaType.APPLICATION_JSON);
							else representation = new StringRepresentation("{\"service\":\"Selected device is not in the list.\"}", MediaType.APPLICATION_JSON);
						}
				}else {
					if(getReference().getRemainingPart().contains("callback")) representation = new StringRepresentation("actionsUpnp({\"service\": \"The number of device is 0.\"})", MediaType.APPLICATION_JSON);
					else representation = new StringRepresentation("{\"service\":\"The number of device is 0.\"}", MediaType.APPLICATION_JSON);
				}
