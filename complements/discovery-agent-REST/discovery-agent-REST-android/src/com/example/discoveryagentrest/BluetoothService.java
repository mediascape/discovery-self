			...
			public void onReceive(Context ctxt, Intent intent) {
		    	if(intent.getAction().equals(BluetoothDevice.ACTION_FOUND))
		    	{
		    		...
		    		if (!listBluetooth.contains(device)) {
			    		Log.d("MyApp", "Add Device: "+device.getName()+" "+device.getType()+" "+device.getAddress()+" "+device.getBondState()+" "+device.getBluetoothClass());
		    		...
		    		if (pairedDevices.size() > 0) {
		    		    ...
		    		    for (BluetoothDevice device1 : pairedDevices) {
		    		    	...
		    		    	Log.d("MyApp", device1.getName()+", "+device1.getType()+", "+device1.getAddress());
		    		    }
		    		}
		    	...
		    	}else if(intent.getAction().equals(BluetoothDevice.ACTION_UUID))
		    		{
			    		...
			    		Log.d("MyApp", "Device: "+device.getName());
			    		if(uuidExtra!=null){
			    			for (Parcelable parcelable : uuidExtra) {
			    				Log.d("MyApp", "Service: "+parcelable.toString());
			    				ParcelUuid parcelUuid = (ParcelUuid) parcelable;
			    				listBluetoothServices.add(parcelUuid);
			    			...
			    			listBluetoothServices.clear();
				    			...
				    			if(!bluetoothSensor.isDiscovering()){
				    				for (int i=0; i<listBluetooth.size(); i++) {
				    					listBluetooth.get(i).fetchUuidsWithSdp();
				    				}
