...
public class BatteryExtraResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		String answer="";
		Log.d("MyApp", "BatteryExtra");
		try {
			JSONObject mainObj = new JSONObject();
			JSONArray ja = new JSONArray();
			JSONObject jo = new JSONObject();
			if(ServiceBoot.batterySensor){
				jo.put("level", ServiceBoot.level+"%");
				ja.put(jo);
				mainObj.put("extra", ja);
			}else{
				mainObj.put("extra", "Error geting battery information.");
			}

			if(getReference().getRemainingPart().contains("callback")) answer="extraBattery("+mainObj.toString()+")";
