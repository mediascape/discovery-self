...
public class CameraExtraResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		String answer="";
		Log.d("MyApp", "CameraExtra");
		try {
			JSONObject mainObj = new JSONObject();
			JSONArray ja = new JSONArray();
			JSONObject jo = new JSONObject();
			if(ServiceBoot.cameraSensor){
				jo.put("cameras", ServiceBoot.cameraNumber);
				ja.put(jo);
				mainObj.put("extra", ja);
			}else{
				mainObj.put("extra", "Error geting number of cameras.");
			}

			if(getReference().getRemainingPart().contains("callback")) answer="extraCamera("+mainObj.toString()+")";
