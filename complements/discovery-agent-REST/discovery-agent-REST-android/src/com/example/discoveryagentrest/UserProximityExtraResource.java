...
public class UserProximityExtraResource extends ServerResource {
	/*
	 * toString
	 * 
    ...
    public Representation doGet() {
		...
		String answer="";
		Log.d("MyApp", "UserProximityExtra");
		try {
			JSONObject mainObj = new JSONObject();
			JSONArray ja = new JSONArray();
			JSONObject jo = new JSONObject();		
			if(ServiceBoot.proximitySensor != null){
				jo.put("near", ServiceBoot.near);
				ja.put(jo);
				mainObj.put("extra", ja);
			}else{
				mainObj.put("extra", "Error geting UserProximity information.");
			}

			if(getReference().getRemainingPart().contains("callback")) answer="extraUserProximity("+mainObj.toString()+")";
